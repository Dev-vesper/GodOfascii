...
// local server, or join one by typing its address. Built from the same
// panel/settings pieces as the in-game Menu; owns selection state and the
// join address text only -- everything else comes back as commands.
class StartMenu {
public:
...
private:
    enum class Page : uint8_t { Main, Join, Settings };
